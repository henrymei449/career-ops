...
| `node discovery-report.mjs --payload <run.json> [--out <path>] [--json]` | Renders the human review Markdown report for a scheduled discovery run (LinkedIn or Target Companies) from `data/scan-runs.tsv` counters + `data/scan-history.tsv` candidate detail, classified via `location-tier.mjs`, to `reports/discovery/{date}_{kind}.md` |
| `post-title-gate.mjs` (library, no CLI) | Shared tier/location/date/salary/content/country/visa/dedup chain, extracted from `scan.mjs`'s own per-job loop so Lane A (keyword matches) and the semantic-recall Lane B (`recall-relevance.mjs`, after a HIGH verdict) run through identical logic — one implementation, not two that can drift |
| `recall-store.mjs` (library, no CLI) | Read/write/lock helpers for the Lane B holding pen `data/recall-candidates.jsonl` (JSONL, not TSV — safely round-trips multiline JD text); deterministic (freshness + per-company/per-source cap) candidate selection; claim/commit helpers for lock-scoped, LLM-call-free critical sections |
| `node recall-relevance.mjs [--recall-eval-cap N] [--per-company-cap N] [--per-source-cap N] [--cli <name>] [--model <name>] [--dry-run] [--json]` | Lane B's cheap, facts-only relevance judge — batches pending `data/recall-candidates.jsonl` rows (title/company/location only, no JD text) through a headless CLI (reuses `rank-pipeline.mjs`'s `CLI_CANDIDATES`/`detectCli`), promotes only HIGH-confidence verdicts through `post-title-gate.mjs` into the same `appendToPipeline`/`appendToScanHistory` path Lane A uses; logs run counters + estimated cost to `data/recall-runs.tsv` |
