...
async function main() {
  ...
  if (result.counts.parsed === 3) pass('all 3 pasted cards parsed');
  else fail(`expected 3 parsed cards, got ${result.counts.parsed}`);
  ...
  if (result.batch_id) pass('exactly one Review batch created for the qualified survivor');
  else fail('expected a batch_id for the qualified survivor');

  const batchView = ui.listReviewJobsForBatch(result.batch_id, root);
  const batchJobs = batchView?.jobs || [];
  if (batchJobs.length === 1 && batchJobs[0].company === 'Acme Manufacturing') pass('Review batch contains ONLY the qualified survivor — no premature/extra admissions');
  else fail(`unexpected batch contents: ${JSON.stringify(batchJobs.map((j) => j.company))}`);
  ...
  if (acme2 && acme2.outcome !== 'added') pass('already-qualified survivor is not re-admitted on re-paste');
  else fail(`Acme Manufacturing re-admitted on re-paste: ${JSON.stringify(acme2)}`);
}
